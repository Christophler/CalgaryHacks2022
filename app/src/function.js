function distanceCalculations(distance) {
  ...
  var object = {
    car: {
      activityName: 'Car',
      timeString: ((distance / speedLimit) * 60).toString() + ' mins',
      costString: '$' + (distance * fuelRate * gasPrice).toString(),
    },
    bike: {
      activityName: 'Bike',
      timeString: ((distance / bikingRate) * 60).toString() + ' mins',
      costString: '$' + calorieCost.toString(),
    },
    scooter: {
      activityName: 'Scooter',
      timeString: ((distance / scooterRate) * 60).toString() + ' mins',
      costString: '$' + (distance * wattUsageRate * wattCostRate).toString(),
    },
    bus: {
      activityName: 'Bus',
      timeString: ((distance / busAvgSpeed) * 60).toString() + ' mins',
      costString: '$' + TTCAdultFare.toString(),
    },
    walk: {
      activityName: 'Walk',
      timeString: ((distance / walkingRate) * 60).toString() + ' mins',
      costString: '$' + calorieCost.toString(),
    },
  };
